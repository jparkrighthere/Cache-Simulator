...
void access_data(mem_addr_t addr) {
    int t_bit = addr >> (s + b);
    int s_bit = (addr >> b) & ((0x1 << s) - 1);

    for (int i = 0; i < E; i++) {
        if ((*(cache + s_bit) + i) -> tag == t_bit) {
            if ((*(cache + s_bit) + i) -> valid == 1) {
                (*(cache + s_bit) + i) -> trackLRU = useLRU++;
                hit_cnt++;
                return;
            }
            else if ((*(cache + s_bit) + i) -> valid == 0) {
                (*(cache + s_bit) + i) -> trackLRU = useLRU++;
    ...
    for (int j = 0; j < E; j++) {
        if ((*(cache + s_bit) + j) -> valid == 0) {
            (*(cache + s_bit) + j) -> trackLRU = useLRU++;
            (*(cache + s_bit) + j) -> tag = t_bit;
    ...
    cache_line_t *LRUcache = & *(*(cache + s_bit) + 0);

    for (int k = 0; k < E; k++) {
        if (LRUcache -> trackLRU > (*(cache + s_bit) + k) -> trackLRU) {
            LRUcache = & *(*(cache + s_bit) + k);
        }
    }
    LRUcache -> trackLRU = useLRU++;
    LRUcache -> valid = 1;
    LRUcache -> tag = t_bit;
}
